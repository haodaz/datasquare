    ...
    return new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // 拼音兜底：中文精确匹配没找到，或找到但全机构未命中 → 统一走拼音变体
          if (isPureChinese) {
            let needPinyin = pingfangCandidates.length === 0;
            if (!needPinyin && institution) {
              const preCheck = runPingfangDisambiguation(pingfangCandidates, { institution });
              if (!preCheck.allScores.some(s => s.instScore > 0)) needPinyin = true;
            }
            if (needPinyin) {
              const pinyinVariants = generatePinyinVariants(searchName);
              sendEvent('log', { step: 'pingfang', message: `↪️ 中文检索${pingfangCandidates.length === 0 ? '无结果' : '有结果但机构未命中'}，尝试拼音英文名变体: ${pinyinVariants.join(', ')}` });
              const existingIds = new Set(pingfangCandidates.map((c: any) => (c as any).id));
              for (const variant of pinyinVariants) {
                try {
                  const alt = await talentService.searchTalents(variant, 5);
                  if (alt.length > 0) {
                    const newOnes = alt.filter((c: any) => !existingIds.has((c as any).id));
                    if (newOnes.length > 0) {
                      newOnes.forEach((c: any) => existingIds.add((c as any).id));
                      sendEvent('log', { step: 'pingfang', message: `✅ 用拼音变体 "${variant}" 找到 ${alt.length} 条候选 (新增 ${newOnes.length} 条)` });
                      pingfangCandidates = [...pingfangCandidates, ...newOnes];
                      sendEvent('log', { step: 'pingfang', message: `🔀 当前共 ${pingfangCandidates.length} 条候选` });
                    } else {
                      sendEvent('log', { step: 'pingfang', message: `↩️ 拼音变体 "${variant}" 命中但无新增候选，继续下一个变体...` });
                    }
                  }
                } catch (e) { /* 单个变体失败继续 */ }
              }
          ...
          if (pingfangCandidates.length === 0) {
            sendEvent('log', { step: 'pingfang', message: `❌ 未找到匹配结果。` });
            pfConfidence = 'none';
          } else {
            const disResult = runPingfangDisambiguation(pingfangCandidates, { institution });
          ...
          // Stage 2: Scholar
          // #修复1: Pingfang 高置信度才用它的 name_en 做 query，否则退回 en_name 或 searchName
          let topScholarInstScore = 0;
          let scholarQuery: string;
          if (pfConfidence === 'high' && topPingfangRecord?.name_en) {
            scholarQuery = topPingfangRecord.name_en as string;
          } else {
            scholarQuery = en_name || searchName;
          }
          // #修复3: 无 en_name 且 searchName 是中文时，用拼音第一个变体做更精准的 query
