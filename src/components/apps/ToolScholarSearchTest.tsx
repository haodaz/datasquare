...
export function ToolScholarSearchTest() {
  const [loading, setLoading] = useState(false);
  const [params, setParams] = useState({
    name_en: 'Binghua Jiang',
    institution: 'Jefferson',
    resume_claims_text: 'h-index 53, total citations 11144, published in Nature...'
  });
  ...
  const handleTest = async () => {
    ...
    setErrorMsg('');
    try {
      const res = await fetch('/api/tools-tester/scholar-search', {
        method: 'POST',
  ...
  return (
    <Card 
      title={<span style={{ display: 'flex', alignItems: 'center', gap: 8 }}><GraduationCap size={18} /> Google Scholar 检索子 AI (Scholar Sub-Agent)</span>}
      style={{ borderRadius: 12, boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}
    >
      <div style={{ marginBottom: 16 }}>
        <p style={{ color: '#64748b', fontSize: 13, marginBottom: 16 }}>
          此沙盒用于测试 <strong>tool_scholar_search</strong>。获取学者学术影响力指标并进行 AI 交叉验证。
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        ...
        <div>
          <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#475569', marginBottom: 4 }}>机构消歧 (institution)</label>
          <Input 
            value={params.institution} 
            onChange={e => setParams({...params, institution: e.target.value})} 
            placeholder="机构名, 如: Jefferson"
          />
        </div>
        <div style={{ gridColumn: '1 / span 2' }}>
          <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#475569', marginBottom: 4 }}>需核验的简历影响力声明 (选填)</label>
          <TextArea 
            rows={3} 
            value={params.resume_claims_text} 
            onChange={e => setParams({...params, resume_claims_text: e.target.value})} 
            placeholder="如: h-index=100, 引用过万 (大模型从原文提取)"
          />
        </div>
      </div>
        ...
        <div style={{ flex: '2 1 400px', background: '#fff', border: '1px solid #e8e8e8', padding: 24, borderRadius: 8, maxHeight: 600, overflowY: 'auto' }}>
          ...
          {result ? (
            <>
              ...
              <div>
                <strong>Google Scholar 实体数据 (Raw Data Sample):</strong>
