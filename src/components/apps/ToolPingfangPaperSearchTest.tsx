...
export function ToolPingfangPaperSearchTest() {
  const [loading, setLoading] = useState(false);
  const [params, setParams] = useState({
    name_cn: '江秉华',
    name_en: 'Binghua Jiang',
    target_papers_json: '[\n  { "title": "Ablation of FAK from lung epithelial cells...", "idx": 1 },\n  { "title": "PI3K/AKT/mTOR pathways...", "idx": 2 }\n]'
  });
  ...
  const handleTest = async () => {
    ...
    try {
      let parsedPapers = [];
      try {
        parsedPapers = JSON.parse(params.target_papers_json);
      } catch (e) {
        throw new Error('论文JSON格式不正确，请检查！');
      }

      const res = await fetch('/api/tools-tester/pingfang-paper-search', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name_cn: params.name_cn,
          name_en: params.name_en,
          target_papers: parsedPapers
        })
      });
  ...
  return (
    <Card 
      title={<span style={{ display: 'flex', alignItems: 'center', gap: 8 }}><FileText size={18} /> 平方论文查询子 AI (Pingfang Paper Sub-Agent)</span>}
      style={{ borderRadius: 12, boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}
    >
      <div style={{ marginBottom: 16 }}>
        <p style={{ color: '#64748b', fontSize: 13, marginBottom: 16 }}>
          此沙盒用于测试 <strong>tool_pingfang_paper_search</strong> (论著验证)。大模型会从简历中提取出多篇代表作，批量派发给此工具进行验真，并交叉校验作者排名。
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        ...
        <div style={{ gridColumn: '1 / span 2' }}>
          <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#475569', marginBottom: 4 }}>待核验代表作 (JSON 数组)</label>
          <TextArea 
            rows={5} 
            value={params.target_papers_json} 
            onChange={e => setParams({...params, target_papers_json: e.target.value})} 
            placeholder="传入带 title 的 json 数组..."
          />
        </div>
      </div>
        ...
        <div style={{ flex: '2 1 400px', background: '#fff', border: '1px solid #e8e8e8', padding: 24, borderRadius: 8, maxHeight: 600, overflowY: 'auto' }}>
          ...
          {result ? (
            <>
              ...
              <div>
                <strong>匹配的论文数据 (Raw Data Sample):</strong>
