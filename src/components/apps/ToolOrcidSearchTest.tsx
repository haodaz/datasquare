...
export function ToolOrcidSearchTest() {
  const [loading, setLoading] = useState(false);
  const [params, setParams] = useState({
    name_en: 'Binghua Jiang',
    keywords: 'Jefferson',
    resume_timeline_text: '2001-2015 Assistant Professor at Jefferson University'
  });
  ...
  const handleTest = async () => {
    ...
    setErrorMsg('');
    try {
      const res = await fetch('/api/tools-tester/orcid-search', {
        method: 'POST',
  ...
  return (
    <Card 
      title={<span style={{ display: 'flex', alignItems: 'center', gap: 8 }}><Globe size={18} /> ORCID 履历检索子 AI (ORCID Sub-Agent)</span>}
      style={{ borderRadius: 12, boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}
    >
      <div style={{ marginBottom: 16 }}>
        <p style={{ color: '#64748b', fontSize: 13, marginBottom: 16 }}>
          此沙盒用于测试 <strong>tool_orcid_search</strong>。获取学者客观履历并进行 AI 交叉验证。
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        <div>
          ...
          <Input 
            ...
            placeholder="必须传入, 如: Binghua Jiang"
          />
        </div>
        <div>
          <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#475569', marginBottom: 4 }}>消歧关键字 (keywords)</label>
          <Input 
            value={params.keywords} 
            onChange={e => setParams({...params, keywords: e.target.value})} 
            placeholder="机构名或研究领域, 如: Jefferson"
          />
        </div>
        <div style={{ gridColumn: '1 / span 2' }}>
          <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#475569', marginBottom: 4 }}>需核验的简历时间线声明 (选填)</label>
          <TextArea 
            rows={3} 
            value={params.resume_timeline_text} 
            onChange={e => setParams({...params, resume_timeline_text: e.target.value})} 
            placeholder="如: 2001-2015 担任某大学教授 (大模型从原文提取)"
          />
        </div>
      </div>
        ...
        <div style={{ flex: '2 1 400px', background: '#fff', border: '1px solid #e8e8e8', padding: 24, borderRadius: 8, maxHeight: 600, overflowY: 'auto' }}>
          ...
          {result ? (
            <>
              ...
              <div>
                <strong>ORCID 学者实体数据 (Raw Data Sample):</strong>
